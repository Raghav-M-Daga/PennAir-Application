"""
same detector as part 2, just pointed at the harder video.

Only the file and MIN_AREA change, the algorithm is untouched. That is kind of
the point, it moves over without edits.

This one is harder in two ways at once. The background is gravel instead of
grass, and the shapes are filled with gradients instead of flat colour, so
both sides of the gap get squeezed. A flat shape used to read 0.00 against
grass at 14.47, here a gradient shape reads 3.99 against gravel at 9.40. Still
separates, just with way less room.
"""
