"""
same detection as part 2 but the centers come out in 3D.
...
photo on its own cannot tell a small close object from a big far one.

The circle is what fixes that. It is 10 inches in radius, so comparing that
to its radius in pixels gives Z straight away, and the surface is flat so
every shape sits at that same depth.
"""
...
def find_depth(shapes):
    ...
    needed. The circle scores 0.985 and the next best shape only 0.886.

    Then Z = focal length * real radius / radius in pixels, which is just
    similar triangles, the same reason far away things look small.
    """
    for shape in shapes:
...
def run():
    ...
    while True:
        ...
        new_depth = find_depth(shapes)
        if new_depth is not None:
            depth = new_depth

        draw_shapes(frame, shapes, depth)
